...
Refer to the [Architecture.md](file:///home/darki/Developement/DScratch/docs/Architecture.md) file for details. DScratch is a custom rich text editor split into:
*   **WASM C# Layer (`src/DScratch` & `src/DScratch.Client`)**: Holds the internal document representation using CRDTs. It handles all event processing, text editing, styling, and cursor calculation, outputting a list of diffs.
*   **TypeScript Layer (`src/DScratch.Client/wwwroot/js`)**: Directly accesses Browser APIs, intercepts input events (via `beforeInput` with `preventDefault`), handles selections/cursors, and applies diffs sent from the C# layer to the DOM.
